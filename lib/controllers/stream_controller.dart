...
class StreamController extends ResourceController {
  ...
  Future<Response> get() async {
    ...
    for(Following flwg in following) {
        ...
        final reschetIschin = ischin.where((element) => element.date.isAfter(DateTime.now().subtract(Duration(days: 1))));
        final oschout = await Future.wait(outgoing.map((x) async => x.then((i) => i)).toList());
        final reschetOschout = oschout.where((element) => element.date.isAfter(DateTime.now().subtract(Duration(days: 1))));
